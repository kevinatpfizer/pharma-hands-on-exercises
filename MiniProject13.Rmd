...
```{r, error=TRUE, message=TRUE}
# MODIFIED
# COMMENT: Reformatted using CTRL+I
library(rio)
library(tidyverse)
library(gt)
library(readr)
...
```{r, error=TRUE, message=TRUE}
# MODIFIED
# COMMENT: Reformatted using CTRL+SHIFT+A
library(rio)
library(tidyverse)
library(gt)
library(readr)
...
## Calculating count for each category of AGEGR1
small_n_cnt = adsl_saf  %>%
  group_by(TRT01AN, TRT01A, AGEGR1) %>%
  count(name = "n")
...
```{r, error=TRUE, message=TRUE}
# MODIFIED
# COMMENT: Tidied up, added function
library(rio)
library(tidyverse)
library(gt)
library(readr)
...
custom_demography <- function(data=adsl_saf, var=SEX){

  BIGNcnt <- {{data}} %>%
  group_by(TRT01AN, TRT01A) %>%
  count(name = "N")
  
  ## Calculating count for each category of the variable specified
  small_n_cnt = adsl_saf  %>%
  ...
  # Pack the outputs into a list
  output_list <- vector(mode = c('list'))
  output_list[[1]] <- adsl_mrg_cnt
...
summary_SEX <- custom_demography(data=adsl_saf, var=SEX)
summary_AGEGR1 <- custom_demography(data=adsl_saf, var=AGEGR1)
summary_RACE <- custom_demography(data=adsl_saf, var=RACE)
