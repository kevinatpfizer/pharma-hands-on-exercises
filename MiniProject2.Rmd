...
```{r group_by modified}
# Subset data, then obtain summary statistics
adsl_eff %>%
  group_by(TRT01A) %>%
  count(name="N")
```
...
```{r small_n modified}
# Subset data, the obtain summary statistics
small_n_cnt <- 
  adsl_eff %>%
...
```{r mutate_and_round}
# Compute percentages, then round values
adsl_mrg_cnt %>%
...
```{r recode}
# Generate display with recoded values
adsl_mrg_cnt %>%
  mutate(perc = round((n/N)*100, digits=1)) %>%
  mutate(perc_char = format(perc, nsmall=1)) %>%
  mutate(npct = paste(n, 
...
```{r}
# COMMENT: It seems there is a conflict with chunk #[r create_adsl_saf], where adsl_eff is created instead of adsl_saf.
# COMMENT: These operations will be performed on adsl_eff (already created) and on adsl_saf (to be created).

# Generate display for adsl_eff ---

# Obtain counts for big N
adsl_eff_bigncount <-
  adsl_eff %>%
...
# Obtain counts for small N
adsl_eff_smallncount <-
  adsl_eff %>% 
  # Group by select variables
...
# Join datasets and configure display
adsl_eff_join <-
  adsl_eff_smallncount %>% 
  left_join(adsl_eff_bigncount) %>% 
  mutate(percent=round((n/N*100), digits=1)) %>% 
  mutate(percent_char=format(percent, nsmall=1)) %>% 
  mutate(npercent=paste(n, '(', percent_char, ')')) %>% 
  mutate(SEX=recode(SEX, 'M'='Male', 'F'='Female')) %>% 
  ungroup() %>% 
  select(TRT01A, AGEGR1, SEX, npercent)

# Transpose data
adsl_eff_transpose <-
  adsl_eff_join %>%
  pivot_wider(names_from=c(TRT01A), values_from=npercent)  
...
```{r}
# Generate display for adsl_saf ---

# Subset data
adsl_saf <-
  adsl %>% 
  filter(casefold(SAFFL, upper=TRUE) == "Y")
...
# Define dataset path
adae_path <- c("https://github.com/phuse-org/phuse-scripts/raw/master/data/adam/cdisc/adae.xpt")

# Read ADAE
adae <- read_xpt(file=adae_path)
...
# Obtain counts for big N
adae_saffl_bigncount <-
  adae_saffl %>%
  group_by(TRTAN, TRTA) %>% 
  count(name = "N")

# Obtain counts for small N
adae_saffl_smallncount <-
  adae_saffl %>%
...
adae_saffl_join <-
  adae_saffl_smallncount %>% 
  left_join(adae_saffl_bigncount) %>% 
  mutate(Percent = as.character(round(x = n/N*100, digits = 1))) %>%
  mutate(npercent = paste0(n, " (", Percent, ")")) %>%
  ungroup() %>% 
  select(-c("N", "n", "Percent", "TRTAN")) %>% 
  pivot_wider(names_from=c(TRTA), values_from=npercent)
