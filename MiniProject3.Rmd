...
```{r}
# Obtain summary statistics
# COMMENT: Number of decimal places for the mean value is set to 1 but the instructions are saying to set to zero.
# COMMENT: Default value was not changed for the number of decimal places for the mean value
age_stat<- adsl_eff %>%
  group_by(TRT01AN,TRT01A,SEX) %>%
  summarize(mean = (mean(AGE) %>% round(digits=1) %>% format(nsmall=1)),
            sd =   (sd(AGE) %>% round(digits=1) %>% format(nsmall=1)), 
            med =  (median(AGE) %>% round(digits=0) %>% format(nsmall=0)),              
            min =  (min(AGE) %>% round(digits=0) %>% format(nsmall=0)), 
            max =  (max(AGE) %>% round(digits=0) %>%  format(nsmall=0)),
            n =    (n() %>% format(nsmall=0)))
...
```{r}
# COMMENT: Instructions say to generate 'age_stat2' but an object names 'age_stat' is being created
# COMMENT: Default value was not changed
age_stat %>%
...
# Inspect structure - grouping variables are TRT01AN, TRT01A, SEX; target values are in rawdata$PARAM==c("HEIGHT (CM)", WEIGHT (CM)")
glimpse(advs)

# Inspect unique values of PARAM
unique(advs$PARAM)

# Subset data (SAFFL=='Y' is used)
advs_subset <- advs %>%
  filter(PARAM == "Height (cm)" | PARAM == "Weight (kg)") %>%
  filter(SAFFL == "Y")

# Summarize height only - long format
heightdata_long <- advs_subset %>%
  filter(PARAM == "Height (cm)") %>%
  select(TRTA, SEX, PARAM, AVAL) %>%
  ungroup() %>%
  group_by(TRTA, SEX, PARAM) %>%
  summarize(Mean = mean(AVAL) %>% round(digits = 1) %>% format(nsmall = 1),
            SD = sd(AVAL) %>% round(digits = 1) %>% format(nsmall = 1),
            Min = min(AVAL) %>% round(digits = 0) %>% format(nsmall = 0),   
            Max = max(AVAL) %>% round(digits = 0) %>% format(nsmall = 0),
            n = n() %>% round(digits = 0) %>% format(nsmall = 0)) %>%
  mutate(Range = paste0("(", Min, ", ", Max, ")")) %>%
  pivot_longer(-c(TRTA, SEX, PARAM), names_to = "Category", values_to = "Value")

# Summarize height only - wide format; CAUTION - make placeholder name for blank values of TRTA
heightdata_wide <- heightdata_long %>%
  ungroup() %>%
  mutate(TRTA = dplyr::case_when(TRTA == "" ~ "No group name",
                                 TRUE ~ as.character(TRTA))) %>%
  pivot_wider(names_from = c(TRTA), values_from = Value)

# Summarize weight only - long format
weightdata_long <- advs_subset %>%
...
# Summarize weight only - wide format; CAUTION - make placeholder name for blank values of TRT01A
weightdata_wide <- weightdata_long %>%
  ungroup() %>%
  mutate(TRTA = dplyr::case_when(TRTA == "" ~ "No group name",
                                 TRUE ~ as.character(TRTA))) %>%
  # pivot_wider(names_from = TRTA, values_from = Value)  # Incorrect line
  pivot_wider(names_from = c(TRTA), values_from = Value)

# Combine the two wide tables
finaldata <- rbind(weightdata_wide, heightdata_wide)
...
```{r}
# View intermediate dataset 1
heightdata_long
```

```{r}
# View intermediate dataset 2
heightdata_wide
```

```{r}
# View intermediate dataset 3
weightdata_long
```

```{r}
# View intermediate dataset 4
weightdata_wide
```
