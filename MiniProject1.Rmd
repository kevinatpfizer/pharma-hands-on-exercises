...
```{r added following warning message}
# #  This function has already been run on 20250207 1749H
# rio::install_formats()
```
...
# Store location in an object
adsl_url <- c("https://github.com/phuse-org/phuse-scripts/raw/master/data/adam/cdisc/adsl.xpt")

# Read ADSL
adsl <- rio::import(file=adsl_url)
```
...
```{r view_adsl_data_types modified}
# Iterate inspection of class over all variables
sapply(X = adsl, FUN = class)
```
...
```{r check_variable_types modified}
...
```{r using_pipe modified}
# Filter data, then sort data
sort_adsl_eff <- adsl %>%
  filter(EFFFL == "Y") %>%
  arrange(USUBJID)
```
...
```{r adsl_saf_srt_select modified}
# Subset data
adsl_eff_srt <- adsl_eff_srt %>%
...
```{r rename_adsl_eff_srt modified}
# Rename variables
...
```{r display_adsl_eff_srt modified}
# Print data
print(adsl_eff_srt)
```
